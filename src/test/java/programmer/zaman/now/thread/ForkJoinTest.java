...
public class ForkJoinTest {
    ...
    void create() {
        ...
        var executorService1 = Executors.newWorkStealingPool();
        var executorService2 = Executors.newWorkStealingPool(5);
    ...
    public static class TotalRecursiveTask extends RecursiveTask<Long> {
        ...
        protected Long compute() {
            if (integers.size() <= 10) {
                // compute
                return doTotalCompute();
            } else {
                // fork
                return forkTotalCompute();
            }
        }

        private Long forkTotalCompute() {
            List<Integer> integers1 = this.integers.subList(0, this.integers.size() / 2);
            List<Integer> integers2 = this.integers.subList(this.integers.size() / 2, this.integers.size());

            TotalRecursiveTask total1 = new TotalRecursiveTask(integers1);
            TotalRecursiveTask total2 = new TotalRecursiveTask(integers2);

            ForkJoinTask.invokeAll(total1, total2);

            return total1.join() + total2.join();
        }

        private Long doTotalCompute() {
